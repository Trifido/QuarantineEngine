...
{
    glUniform2fv(glGetUniformLocation(ID, name.c_str()), 1, &pos[0]);
}
...
void Shader::setMat4(const std::string& name, float mat[16]) const
{
	glUniformMatrix4fv(glGetUniformLocation(ID, name.c_str()), 1, GL_FALSE, &mat[0]);
}

void Shader::setMat4(const std::string& name, glm::mat4& mat, unsigned int nMat) const
{
    glUniformMatrix4fv(glGetUniformLocation(ID, name.c_str()), nMat, GL_FALSE, &mat[0][0]);
}
